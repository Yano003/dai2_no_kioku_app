...
class ScheduleParser {
  ...
  /// 時刻の前に付く時間帯の語。
  ///
  /// 「午前・午後」だけでなく、話し言葉の「朝・昼・夕方・夜・晩」も受ける。
  /// テスト後アンケート（2026/09/04）で「明日8時」と登録された方が夜20時の
  /// つもりだった件への対応。「夜8時」と言えば20時として読めるようにする。
  /// あわせて、これらの語が予定名の側に残らなくなる。
  /// （「朝9時に病院で採血」→ 予定名が「朝 病院で採血」になっていた）
  ///
  /// 「夕方」は「夕」より先に並べること。先に「夕」と一致すると
  /// 「方」が予定名に残る。
  static const _meridiem = r'午前|午後|朝|昼|夕方|夕|夜|晩';

  /// 後ろの時刻を午後として読む語。「夕方5時」は17時。
  /// 12時台（「昼12時」）はそのまま12時とする。
  static const _pmWords = {'午後', '昼', '夕方', '夕', '夜', '晩'};

  static final _timePatterns = [
    RegExp('($_meridiem)?' r'\s*(\d{1,2})\s*時\s*(\d{1,2})\s*分'),
    RegExp('($_meridiem)?' r'\s*(\d{1,2})\s*時(?!間)'),
  ...
  _TimeMatch? _extractTime(String text) {
    for (var i = 0; i < _timePatterns.length; i++) {
      ...
      var hour = rawHour;
      if (meridiem != null && _pmWords.contains(meridiem) && hour < 12) {
        hour += 12;
      }
      // 「午前12時」は0時。話し言葉の「朝12時」は昼の12時を指すことが
      // 多いため、こちらは0時に倒さない。
  ...
  static String _cleanTitle(String text) {
    ...
    // 助詞として現れる語だけを対象にする。
    //
    // 「から」はテスト後アンケート（2026/09/04）で4人中2人が手直しを
    // 強いられた語。「明日17時から打ち合わせ」から時刻を取り除くと
    // 「から打ち合わせ」が残ってしまうため、対になる「まで」「より」と
    // あわせて落とす。
    final leading =
        RegExp(r'^[\s、。,\.]*(?:から|まで|より|[にはのを])?[\s、。,\.]*');
    String previous;
    do {
      previous = title;
