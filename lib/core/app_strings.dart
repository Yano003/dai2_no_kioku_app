...
class AppStrings {
  ...
  ///
  /// かぎ括弧から山括弧に改め、カードの名前と同じ青緑で置く。
  /// 「今日の安心カード」という名前の見え方をカード側と揃えるため。
  ...
  // 時刻を選ぶダイアログ
  //
  // 時計盤（Material 既定の showTimePicker）は初見で操作が分からないという
  // 声がテスト後アンケート（2026/09/04）で挙がったため、数字を上下に
  // 動かして選ぶ形に改めた。（クライアントご指示 2026/09/05）
  // ---------------------------------------------------------------------------

  static const timePickerTitle = '時刻を選ぶ';
  static const timePickerHint = '上下に動かして選んでください';
  static const timePickerHourSuffix = '時';
  static const timePickerMinuteSuffix = '分';
  static const timePickerConfirm = '決定';
  static const timePickerCancel = 'やめる';
  ...
  /// 前日夜のカードの締め。10種類から毎晩ひとつを選んで出す。
  /// （ヒトコト株式会社様ご提供 2026/09/05）
  ///
  /// 寝る前に開く方がいるため、その晩をねぎらう一言を添える。
  /// 選び方は [previousNightClosingFor] を参照。
  static const cardPreviousNightClosings = [
    'また明日の朝にお会いしましょう。',
    '今日もお疲れさまでした。',
    '今日はいい1日でしたか？',
    '今日もよくがんばりましたね。',
  ...
  /// その晩に出す締めの一言を選ぶ。
  ///
  /// 日付から決めているため、同じ晩に何度カードを開いても文言は変わらない。
  /// 開くたびに変わると、さきほど読んだ言葉が消えたように見えてしまうため。
  /// 日付が変われば別の一言になり、日付を掛け合わせて散らしているので
  /// 並び順どおりに出ることもない。
  static String previousNightClosingFor(DateTime date) {
    final seed = date.year * 10000 + date.month * 100 + date.day;
    final index = (seed * 1103515245 + 12345) % cardPreviousNightClosings.length;
    return cardPreviousNightClosings[index];
  }
  ...
  /// 前日夜のカードの結び。締めの一言と同じ大きさで置く。
  /// （クライアントご指示 2026/09/05）
  static const cardGoodNight = 'おやすみなさい';

  /// 当日朝のカードの結び。
  ///
  /// 「いってらっしゃい」から改めた。これから出かける方ばかりではなく、
  /// 家で過ごす方もいるため。（クライアントご指示 2026/09/05）
  static const cardHaveANiceDay = 'いい1日を';

  // 当日日中の「確認しました」ボタンは第2.0版で廃止された。（要件定義書 4.5）
  // 実質的にアプリを閉じるだけの操作のため不要と判断されたもの。
  // 完了チェックは各予定のチェックボックスで行う。

  // 「おやすみなさい」「いってらっしゃい」のボタンも 2026/09/05 に廃止した。
  // 押しても何も起きないボタンだったため、押させずに言葉だけを残す。
  // （クライアントご指示 2026/09/05）
  ...
  static const errorSpeechEmpty = 'もう一度お願いします。';
